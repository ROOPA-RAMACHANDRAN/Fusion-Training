...
Acc_desp,((nvl(COST,0)+nvl(ADJ_COST,0)+nvl(cost_retired,0))-(Acc_desp))nbv,dp_amt_current,(nvl(Acc_desp,0) - nvl(YTD_DEPRN,0))Accum_prior from
(
                 ...
                 WHERE     fdd1.asset_id = ass_id
                       AND PERIOD_COUNTER =  
                              (SELECT MAX (PERIOD_COUNTER)
                                 FROM fa_deprn_detail fddi
                                WHERE     fddi.ASSET_ID = FDD1.ASSET_ID
								       and exists (select 1 from FA_DEPRN_PERIODS FDP where
									     FDDI.BOOK_TYPE_CODE = FDP.BOOK_TYPE_CODE
										  AND FDDI.PERIOD_COUNTER = FDP.PERIOD_COUNTER
										  and substr(period_name,-1,2) =substr(:p_period_name,-1,2))  	
                                      AND fddi.distribution_id =
                                             fdd1.distribution_id
                                      AND fddi.distribution_id =
                                             fdhi.distribution_id
                       ...
                       AND FDD1.DISTRIBUTION_ID = FDHI.DISTRIBUTION_ID ),0) YTD_DEPRN,
...
INVOICE_AMOUNT,
Supplier_Name,
COST,period_name,dp_res ,YTD_DEPRN,fiscal_year) MAIN,
...
FA_DEPRN_PERIODS FDP
where a.asset_id=b.asset_id 
 --and b.asset_number  ='580'
and a.ADJUSTMENT_TYPE='COST'
and SOURCE_TYPE_CODE in ('ADDITION')
and  PERIOD_COUNTER_ADJUSTED=PERIOD_COUNTER
and fdp.book_type_code=:P_books
group by asset_number,a.asset_id,PERIOD_COUNTER_ADJUSTED,fdp.PERIOD_NAME, a.SOURCE_TYPE_CODE ,fdp.FISCAL_YEAR) asset_year
