...
CASE WHEN SGD IS NOT NULL THEN ROUND(SGD *  NVL(CURRENT_CONVERSION_RATE,1),2)
	 END                                                                        SGD_CR,
SGD,
USD,
CASE WHEN SGD IS NULL AND USD IS NOT NULL THEN 0
     ELSE (ROUND(SGD *  NVL(CURRENT_CONVERSION_RATE,1),2) - USD )
 END REVAL_USD
FROM
(SELECT 
GL.NAME LEDGER,
...
NVL((SELECT gd.conversion_rate
                   FROM gl_daily_rates gd
                  WHERE 
				    gd.from_currency = GJH.CURRENCY_CODE
                    AND gd.to_currency = 'USD'
                    AND TO_CHAR(GD.CONVERSION_DATE,'Mon-YY','NLS_DATE_LANGUAGE = AMERICAN') = :P_PERIOD_NAME
                    AND gd.CONVERSION_TYPE = '300000001383200'),1 )CURRENT_CONVERSION_RATE,
