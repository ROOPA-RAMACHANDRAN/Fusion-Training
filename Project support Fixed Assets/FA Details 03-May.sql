-- FA Report Detail
	...
	CASE WHEN NVL(A.MNTH_ADJ_DEPN,0) !=0 THEN NVL(A.MNTH_ADJ_DEPN,0)-NVL(A.MNTH_UNPLAN_DEPN,0) ELSE 0 END MNTH_ADJ_DEPN,
	...
	NVL((Case When  row_number() over(partition by A.book_type_code,A.Asset_id order by A.book_type_code,A.Asset_id, A.TRANSACTION_TYPE)=1 Then
	NVL(A.Cost_PRE_YEAR,0) END),0) Cost_PRE_YEAR,   -- Cost B/F
	
	NVL((Case When  row_number() over(partition by A.book_type_code,A.Asset_id order by A.book_type_code,A.Asset_id)=1 Then
	NVL(A.AccDep_PRE_YEAR,0)  END),0) AccDep_PRE_YEAR,  -- AccDep B/F
	
	NVL((Case When  row_number() over(partition by A.book_type_code,A.Asset_id order by A.book_type_code,A.Asset_id)=1 Then
	NVL(A.Recoverable_Cost,0)  END),0) Recoverable_Cost, --- Cost C/F
	
	NVL((Case When  row_number() over(partition by A.book_type_code,A.Asset_id order by A.book_type_code,A.Asset_id)=1 Then
	CASE WHEN NVL(A.Cost_PRE_YEAR,0) >= NVL(A.AccDep_PRE_YEAR,0) THEN NVL(NVL(A.Cost_PRE_YEAR,0) - NVL(A.AccDep_PRE_YEAR,0),0) ELSE 0 END
	  END),0) PRE_NET_BOOK_VALUE, --- NBV B/F
	
	NVL((Case When  row_number() over(partition by A.book_type_code,A.Asset_id order by A.book_type_code,A.Asset_id)=1 Then
	NVL(A.DEPRN_RESERVE,0)  END),0) DEPRN_RESERVE, --- AccDep C/F
	
	NVL((Case When  row_number() over(partition by A.book_type_code,A.Asset_id order by A.book_type_code,A.Asset_id)=1 Then
	Case When NVL(A.Recoverable_Cost,0) >= NVL(A.DEPRN_RESERVE,0)   THEN NVL(NVL(A.Recoverable_Cost,0) - NVL(A.DEPRN_RESERVE,0),0) ELSE 0 END
	END) ,0) NET_BOOK_VALUE,  --- NBV C/F
	...
	CASE WHEN (FRT.DATE_RETIRED IS NOT NULL) AND FTH.TRANSACTION_TYPE_CODE = 'FULL RETIREMENT' THEN 'Y' ELSE 'N' END Disposal_FLAG,
...
where 1=1
	...
	(CASE WHEN (Unplan_Deprec1 IS not null AND RECOVERABLE_COST1 is NOT NULL) THEN RECOVERABLE_COST 
		WHEN (Unplan_Deprec1 IS null AND RECOVERABLE_COST1 is NOT NULL) THEN RECOVERABLE_COST
	ELSE Unplan_Deprec END)
	...
	CASE WHEN (RECOVERABLE_COST1 IS not null AND Unplan_Deprec1 is NOT NULL ) THEN RECOVERABLE_COST1 
		WHEN (RECOVERABLE_COST1 IS not null AND Unplan_Deprec1 is NULL ) THEN RECOVERABLE_COST1
		ELSE Unplan_Deprec1 END As SALVAGE_VALUE
	FROM
	(
	Select 
...
where 1=1
	...
	Where 1=1 
	---Parameter
	  AND NVL(A.Posting_Date , sysdate+1) Between NVL(:P_POSTING_FROM,NVL(A.Posting_Date , sysdate-1)) 
			AND NVL(:P_POSTING_TO,NVL( A.Posting_Date , sysdate+2))
	
--AND A.ASSET_NUMBER = '1'
	
	Order by A.ASSET_NUMBER , A.Document_Number , DEPRECIATION_PERIOD ASC , A.TRANSACTION_DATE DESC